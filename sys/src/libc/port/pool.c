...
/* unsafe macro */
#define Abovemin(n, bsz, p) \
	((n) >= (bsz)>>2 && (n) >= MINBLOCKSIZE && (n) >= (p)->minblock)
...
blocksetdsize(Pool *p, Alloc *b, uintptr dsize)
{
	...
	/* must fit in ushort for t->datasize */
	assert(b->size - dsize < Ushortsz);
...
trim(Pool *p, Alloc *b, uintptr dsize)
{
	...
	extra = b->size - bsize;
	/* b->size - dsize must fit in ushort */
	if(b->size - dsize >= Ushortsz || Abovemin(extra, bsize, p)) {
...
freefromfront(Pool *p, Alloc *b, uintptr skip)
{
	...
	skip &= ~(p->quantum-1);
	if(skip >= Ushortsz || Abovemin(skip, b->size, p)) {
