...
enum{
	Bpw=	8*sizeof(uint),
	Busy= 	(uint)1<<(Bpw-1),		/* set in Header.s.size of allocated blocks */
...
{
	struct {
		uint	size;		/* size in units including Header, and Busy flag */
		uint	tag;		/* only for debugging: set malloctag */
...
struct Region
{
	...
	Header*	end;
	uint	size;		/* remaining space, in Unitsz */
	char*	base;
	char*	limit;
};
...
#define	BIGHASH(p)		((uintptr)p%BHashSize)
#define	isbigalloc(p)	((uintptr)(p)>=KSEG2 && (uintptr)(p)<KSEG1)
...
static	void	coalesce(Region*);
static	void	freemisc(Header*);
static	BigAlloc*	bigfind(void*);
static	void	bignote(void*, usize, uintptr);
static	void*	bigalloc(usize, usize, uintptr);
static	void	bigfree(void*);

static void	(*memprof)(void*, ulong, usize, int) = nil;
...
static	void*	qalloc(usize, uintptr);
static	void	qfreeinternal(Header*);
...
static void*
qallocalign(usize nbytes, uintptr align, long offset, usize span, uintptr pc)
{
	Qlist *qlist;
	uintptr aligned;
	Header **pp, *p, *q, *r;
	uint n, nunits, alunits, maxunits, gap;
...
static void*
qalloc(usize nbytes, uintptr pc)
{
	Qlist *qlist;
	Header *p, *q, *t;
	uint nunits, u;
...
static void
bignote(void *p, usize nbytes, uintptr pc)
{
	BigAlloc *b, **l;
...
static void*
bigalloc(usize nbytes, usize align, uintptr pc)
{
	void *p;
...
free(void* ap)
{
	...
	nunits = h->s.size;
	if((nunits & Busy) == 0)
		panic("free: already free %#p: freed %#p tag %#ux", ap, getcallerpc(&ap), h->s.tag);
	nunits &= ~Busy;
	h->s.size = nunits;
	if(nunits < MinUnits)
...
if(size > 1536*1024)print("malloc %lud %#p\n", size, getcallerpc(&size));
	if(size >= BigThreshold)
		v = bigalloc(size, 0, getcallerpc(&size));
	else
		v = qalloc(size, getcallerpc(&size));
	if(v != nil)
		memset(v, 0, size);
	return v;
}
...
if(nbytes > 1900*1024)print("mallocalign %lud %lud %#p\n", nbytes, align, getcallerpc(&nbytes));
	...
	if(nbytes >= BigThreshold)
		v = bigalloc(nbytes, align, getcallerpc(&nbytes));
	else
		v = qallocalign(nbytes, align,offset, span, getcallerpc(&nbytes));
	if(v != nil){
...
setmalloctag(void *a, ulong tag)
{
	...
	h = D2H(a);
	if((h->s.size & Busy) == 0)
		panic("setmalloctag free %#p %#lux [%#ux %#ux] %#p", a, tag, h->s.size, h->s.tag, getcallerpc(&a));
	h->s.tag = tag;
	if(memprof != nil)
...
void
setmemprof(void (*f)(void*, ulong, usize, int))
{
	memprof = f;
}
