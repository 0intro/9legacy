...
void
memmaprange(uintptr va, uintmem lo, uintmem hi, PTE (*alloc)(usize), PTE flags)
{
	uintmem mem, nextmem;
	PTE *pte, f;
	int i, l;
...
meminit(void)
{
	...
	for(asm = rmapram.map; asm != nil; asm = asm->next){
		va = KSEG2+asm->addr;
		lo = asm->addr;
...
void
memaffinity(u64int base, u64int len, u32int dom, int flags)
{
...
void*
basealloc(usize nb, uint align, usize *alloced)
{
	uintmem pa;
...
void
asmmodinit(u32int start, u32int end, char*)
{
