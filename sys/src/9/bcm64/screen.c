...
void
fbsetup(char *prop, uchar *value, int len)
{
	if(!strcmp(prop, "width") && len == 4)
		xgscreen.r.max.x = nhgetl(value);
	else if(!strcmp(prop, "height") && len == 4)
		xgscreen.r.max.y = nhgetl(value);
	else if(!strcmp(prop, "reg") && len == 8){
		fbaddr = nhgetl(value);
		fbsize = nhgetl(value+4);
	}else if(!strcmp(prop, "format")){
		iprint("framebuffer format %s\n", (char*)value);
		if(value[0] == 'a')
			/* no alpha channel - change a8r8g8b8 to x8r8g8b8 */
			value[0] = 'x';
		fbchan = strtochan((char*)value);
		if(fbchan == RGB24)
			fbchan = BGR24;
	}
}
...
screeninit(void)
{
	...
	if(fbchan == 0 || fbaddr == 0 || fbsize == 0){
		print("missing framebuffer configuration from devicetree\n");
		return;
	}
	if(memsetchan(&xgscreen, fbchan) == -1){
		print("bad framebuffer format from devicetree\n");
		return;
	}
	fb = (char*)mmukmap(FRAMEBUFFER, fbaddr, fbsize);
	print("screen %d x %d x %d fbinit %#p\n", xgscreen.r.max.x, xgscreen.r.max.y, xgscreen.depth, fb);
	memset(fb, 0x55, fbsize);
	xgscreen.clipr = xgscreen.r;
	conf.monitor = 1;
	xgdata.bdata = (uchar*)fb;
	xgdata.ref = 1;
	gscreen = &xgscreen;
