 ...
 *
 * Raspberry Pi 5:
 *	eia0 is debug uart (JST connector between HDMI sockets)
 *	eia1-5 are RP1 uarts accessible from 40-pin header
 */
...
static Pinctl pinctl[] = {
	{  0, 0, 0xFF},		/* debug pins */
	{ 14, 15, 4, },		/* rpi pins  8, 10 */
	{  0,  1, 2, },		/* rpi pins 27, 28 */
	{  4,  5, 2, },		/* rpi pins  7, 29 */
	{  8,  9, 2, },		/* rpi pins 24, 21 */
	{ 12, 13, 2, },		/* rpi pins 32, 33 */
};
...
/*
* pi5 - start with the debug uart as eia0
*/
static Ctlr pl011ctlr[] = {
{	.io	= (u32int*)(0x107d001000ull), //(VIRTIO+0x7d001000),
	.irq	= IRQpl011,
	.poll	= 0, },
{	.io	= (u32int*)(VIRTIO+0x80030000ull),
	.irq	= IRQuart1,
	.poll	= 0, },
{	.io	= (u32int*)(VIRTIO+0x80034000ull),
	.irq	= IRQuart2,
	.poll	= 0, },
{	.io	= (u32int*)(VIRTIO+0x80038000ull),
	.irq	= IRQuart3,
	.poll	= 0, },
{	.io	= (u32int*)(VIRTIO+0x8003c000ull),
	.irq	= IRQuart4,
	.poll	= 0, },
{	.io	= (u32int*)(VIRTIO+0x80040000ull),
	.irq	= IRQuart5,
	.poll	= 0, },
};
...
pl011baud(Uart* uart, int baud)
{
	...
	ctlr = uart->regs;
	csr8o(ctlr, Ibrd, uart->freq / bgc);
	csr8o(ctlr, Fbrd, uart->freq % bgc);
	/*
...
pl011enable(Uart* uart, int ie)
{
	...
	p = &pinctl[uart->dev];
	if(p->alt != 0xFF){
		gpioselrp1(p->txpin, p->alt);
		gpioselrp1(p->rxpin, p->alt);
		gpiopulldownrp1(p->txpin);
		gpiopulldownrp1(p->rxpin);
	}
